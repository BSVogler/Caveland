...
public class ExitPortal extends Portal implements Interactable {
	...
	public void update(float dt) {
		...
		if (hasPosition()) {
			if (spawner) {
				...
				if (getPosition().getEntitiesNearby(RenderCell.GAME_EDGELENGTH * 13, Ejira.class).isEmpty() && spawnCooldown <= 0) {
	...
	public Coordinate getGround(){
		...
		//find ground
		while (!Controller.getMap().getBlockConfig().isObstacle(ground.getBlockId())) {
			ground.add(0, 0, -1);
		}
		return ground;
	}
